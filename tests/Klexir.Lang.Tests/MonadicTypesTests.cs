...
public sealed class MonadicTypesTests
{
    ...
    {
        Run("match map(Some(5), fun (x: Int) => x + 1) with Some(x) => x | None => 0")
    ...
    {
        Run("match map(None<Int>, fun (x: Int) => x + 1) with Some(x) => x | None => 99")
    ...
    {
        Run("match map(Ok<Bool>(5), fun (x: Int) => x + 1) with Ok(x) => x | Err(e) => 0")
            .Should().Be(new IntValue(6));
        Run("match map(Err<Int>(true), fun (x: Int) => x + 1) with Ok(x) => x | Err(e) => 99")
            .Should().Be(new IntValue(99));
    }
    ...
    {
        const string chain =
            "match bind(Some(5), fun (x: Int) => if x > 0 then Some(x * 2) else None<Int>) " +
            "with Some(x) => x | None => 0";
        Run(chain).Should().Be(new IntValue(10));

        Run("match bind(None<Int>, fun (x: Int) => Some(x * 2)) with Some(x) => x | None => 99")
            .Should().Be(new IntValue(99));
    }
    ...
    public void Evaluate_bind_chains_railway_oriented_Result_computations_and_short_circuits_on_Err()
    {
        ...
        const string shortCircuit = "match bind(Err<Int>(true), fun (x: Int) => Ok<Bool>(x * 2)) " +
    ...
    {
        Check("map(5, fun (x: Int) => x + 1)").IsFailure.Should().BeTrue();
    }

    [Fact]
    public void Check_fails_when_bind_changes_the_Result_error_type()
    {
        Check("bind(Ok<Bool>(5), fun (x: Int) => Ok<Int>(x))").IsFailure.Should().BeTrue();
    }
